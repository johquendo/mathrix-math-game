...
# UI elements
signal problem_solved(correct: bool, difficulty: int)

# Remove all the @onready references that don't exist
# These nodes are not children of ProblemGenerator in your scene tree
...
func _ready():
	add_to_group("problem_solver")
	
	# Print welcome message to output
	print("=== Math Problem Generator ===")
	print("Math problems will be generated when purchased from shop!")
	print("Problems will appear in the output console.")
	print("=====================================")
...
func check_answer(user_answer):
	var is_correct = user_answer == current_item.answer
	problem_solved.emit(is_correct, current_difficulty)
	return is_correct

# Set difficulty (called from GameManager when buying items)
